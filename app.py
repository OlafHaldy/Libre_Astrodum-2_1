...
ПРАВИЛА (строго):
- ОДНА КОРОТКАЯ СТРОКА (не больше 10-12 слов)
- БЕЗ ПЕРЕНОСОВ
- Иронично, жизненно, в характере знака

Пример:
«Если вы решили уйти от Водолея, не спешите, подождите когда он придержит вам дверь и вызовет лифт.»

Выдай ТОЛЬКО одну короткую строку. Без кавычек, без пояснений."""

    try:
        aphorism = generate(prompt)
        # Берём первую строку
        aphorism = aphorism.split('\n')[0].strip()
        # Обрезаем до 80 символов (жестко!)
        if len(aphorism) > 80:
