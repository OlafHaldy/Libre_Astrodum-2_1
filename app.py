...
def key_v1(sign: str = "Aquarius"):
    ...
    ALL_TOPICS = [
        'порядок и чистота в доме', 'уют и быт', 'ремонт и обустройство',
        'семейный ужин и традиции', 'соседи и границы', 'дедлайн и срочные задачи',
        'отношения с начальником', 'коллеги и коллектив', 'увольнение и перемены',
        'повышение и амбиции', 'первое свидание', 'ссора и примирение',
        'ревность и доверие', 'расставание и отпускание', 'долгая любовь и быт',
        'одиночество и уединение', 'страдание и его смысл', 'судьба и случай',
        'свобода и ответственность', 'смерть и память', 'траты и удовольствия',
        'сбережения и безопасность', 'долги и обязательства', 'крупная покупка',
        'щедрость и скупость', 'предательство и прощение', 'поддержка в трудный час',
        'новая дружба', 'старые друзья', 'дружба и время',
        'усталость и восстановление', 'бессонница и тревога', 'мотивация и воля',
        'тело и дух', 'привычки и изменения'
    ]
...
ЖЁСТКИЕ ПРАВИЛА:
1. РОВНО ОДНА КОРОТКАЯ СТРОКА (максимум 15 слов)
2. БЕЗ ПЕРЕНОСОВ — только сплошной текст
3. Иронично, философски, в характере знака
4. Никаких «вас ждёт», «будьте осторожны»
5. Без воды — только суть

Пример для Водолея:
«Если вы решили уйти от Водолея, не спешите, подождите когда он придержит вам дверь и вызовет лифт.»
...
Выдай ТОЛЬКО одну короткую строку афоризма. Ничего другого."""

    try:
        aphorism = generate(prompt)
        # Берём только первую строку (если AI добавил переносы)
        aphorism = aphorism.split('\n')[0].strip()
        # Если слишком длинный — обрезаем
        if len(aphorism) > 150:
            aphorism = aphorism[:147] + '...'
    except Exception as e:
