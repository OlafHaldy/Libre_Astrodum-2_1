...
def key_v1(sign: str = "Aquarius"):
    ...
    WEEKDAY_TOPICS = {
        0: [  # Понедельник
            'планы и цели', 'дисциплина', 'ответственность',
            'рабочие задачи', 'организация'
        ],
        1: [  # Вторник
            'дедлайн и срочные задачи', 'коллеги и коллектив',
            'амбиции', 'повышение', 'энергия'
        ],
        2: [  # Среда
            'отношения с начальником', 'дружба', 'гармония',
            'баланс', 'совместная работа'
        ],
        3: [  # Четверг
            'карьера', 'деньги', 'перемены',
            'крупная покупка', 'инвестиции'
        ],
        4: [  # Пятница
            'свобода и отдых', 'траты и удовольствия',
            'встреча с друзьями', 'расслабление', 'вечеринка'
        ],
        5: [  # Суббота
            'уют и быт', 'ремонт и обустройство',
            'семейный ужин', 'соседи', 'дом'
        ],
        6: [  # Воскресенье
            'одиночество и уединение', 'судьба и случай',
            'смысл жизни', 'покой', 'размышления'
        ]
    }

    sign_ru = SIGN_NAMES_RU.get(sign, sign)
    
    # Получаем день недели (0=пн, 6=вс) и число месяца
    now = datetime.datetime.now()
    weekday = now.weekday()
    day = now.day
    
    # Смешиваем: берём тему из дня недели + день месяца для вариативности
    weekday_topics = WEEKDAY_TOPICS.get(weekday, ['жизнь и мудрость'])
    # Используем день месяца для выбора конкретной темы из списка
    topic_index = (day - 1) % len(weekday_topics)
    topic = weekday_topics[topic_index]
...
Сегодня {weekday_name}, {day}-е число месяца.

Правила:
- Не больше 10 слов
- Учитывай настроение дня недели
