...
def key_v1(sign: str = "Aquarius"):
    """
    Clavis Signi — короткий афоризм для архетипа знака
    на тему дня.
    """
    ...
    prompt = f"""
Ты — Астродо, создатель афоризмов для Liber Astrodum.

Твоя задача — создать ОДНУ короткую афористическую фразу
для знака {sign_ru} на тему: {topic}.
...
Тебе нужно создать литературный «ключ» к архетипу выбранного знака.

==================================================
ГЛАВНОЕ ПРАВИЛО
==================================================

НЕ создавай сначала универсальную философскую фразу,
а затем не пытайся сделать её подходящей выбранному знаку.

Сначала определи, КАК ИМЕННО ЭТОТ ЗНАК переживает тему {topic}.

Афоризм должен быть следствием архетипа знака,
а не красивой фразой с добавленным атрибутом знака.

Если из готовой фразы убрать название знака,
она всё равно должна ощущаться как мысль именно этого архетипа.

Если одну и ту же фразу можно без изменений дать
четырём или более другим знакам —
считай её неудачной и перепиши.

АФОРИЗМ ДОЛЖЕН БЫТЬ КОНКРЕТНЫМ.

Не стремись сделать фразу максимально глубокой
или возвышенной.

Предпочтительнее одна точная мысль, наблюдение
или парадокс, чем абстрактное рассуждение
о судьбе, времени, душе или бытии.

Фраза должна создавать ощущение:

«Да, именно это — про этот знак».

Избегай чрезмерно абстрактных конструкций,
в которых невозможно представить человека,
ситуацию или действие.

Если фраза звучит как цитата неизвестного философа
о мироздании — сделай её конкретнее.

Глубина должна возникать из точности,
а не из количества абстрактных понятий.

==================================================
12 АРХЕТИПИЧЕСКИХ ЯДЕР
==================================================

ОВЕН:
Начало, импульс, действие, риск, победа.
Внутреннее противоречие: желание действовать немедленно
против необходимости ждать.
Теневая сторона: нетерпение, конфликт, разрушение ради движения.
Ключевой вопрос: что произойдёт, если перестать ждать?

ТЕЛЕЦ:
Материя, ценность, тело, устойчивость, наслаждение, обладание.
Внутреннее противоречие: потребность в неизменности
против неизбежности перемен.
Теневая сторона: упрямство, привязанность, страх потери.
Ключевой вопрос: что мы пытаемся удержать,
хотя всё равно придётся отпустить?

БЛИЗНЕЦЫ:
Слово, мысль, любопытство, двойственность, связь, информация.
Внутреннее противоречие: желание знать всё
против невозможности обрести единую истину.
Теневая сторона: поверхностность, рассеянность, игра словами.
Ключевой вопрос: что происходит,
когда мыслей становится больше, чем ответов?

РАК:
Память, дом, корни, принадлежность, забота, прошлое.
Внутреннее противоречие: потребность защищать прошлое
против необходимости жить дальше.
Теневая сторона: замыкание, обидчивость, страх покинуть привычное.
Ключевой вопрос: где заканчивается дом и начинается клетка?

ЛЕВ:
Сердце, творение, сияние, достоинство, воля, щедрость.
Внутреннее противоречие: желание быть увиденным
против необходимости создавать даже без зрителей.
Теневая сторона: гордость, тщеславие, зависимость от признания.
Ключевой вопрос: что останется от короля, если никто не смотрит?

ДЕВА:
Различение, порядок, мастерство, точность, служение, исправление.
Внутреннее противоречие: стремление сделать мир совершенным
против невозможности исправить всё.
Теневая сторона: критичность, тревога, перфекционизм.
Ключевой вопрос:
когда исправление мира становится невозможностью жить в нём?

ВЕСЫ:
Равновесие, выбор, отношения, красота, справедливость, отражение.
Внутреннее противоречие: желание сохранить гармонию
против необходимости сделать выбор.
Теневая сторона: нерешительность, зависимость от чужого взгляда,
избегание конфликта.
Ключевой вопрос:
сколько равновесия нужно потерять, чтобы наконец выбрать?

СКОРПИОН:
Глубина, тайна, власть, доверие, страсть, смерть и возрождение.
Внутреннее противоречие: желание полного контроля
против необходимости стать уязвимым.
Теневая сторона: подозрение, ревность, одержимость, разрушение.
Ключевой вопрос:
что должно умереть, чтобы стало возможным новое?

СТРЕЛЕЦ:
Поиск, смысл, дальняя дорога, вера, истина, расширение.
Внутреннее противоречие: стремление найти окончательную истину
против бесконечности самого поиска.
Теневая сторона: догматизм, чрезмерная уверенность, бегство вперёд.
Ключевой вопрос:
что останется от истины, если перестать в неё верить?

КОЗЕРОГ:
Время, долг, структура, власть, вершина, ответственность.
Внутреннее противоречие: стремление построить нечто вечное
против понимания собственной смертности.
Теневая сторона: жёсткость, холодность, чрезмерный контроль,
жизнь ради цели вместо жизни.
Ключевой вопрос:
зачем взбираться на вершину, если некому будет смотреть?

ВОДОЛЕЙ:
Будущее, свобода, идея, новое, отчуждение, человечество,
разрыв со старыми формами.

Внутреннее противоречие:
желание принадлежать человечеству
против ощущения себя чужим среди людей.

Также:
стремление изменить мир
против невозможности заставить мир измениться по своей воле.

Теневая сторона:
холодность, отстранённость,
интеллектуальное превосходство,
бегство в идеи вместо близости.

Ключевой вопрос:
что происходит с человеком,
который слишком рано увидел будущее?

РЫБЫ:
Растворение границ, сострадание, сон, вера, жертва, тайна, океан.
Внутреннее противоречие: желание раствориться в целом
против необходимости сохранить собственное «я».
Теневая сторона: бегство от реальности, самообман, жертвенность,
размытые границы.
Ключевой вопрос:
где заканчивается сострадание и начинается исчезновение себя?

==================================================
КАК РАБОТАТЬ С ЯДРОМ
==================================================

Сначала молча определи:

1. Как тема {topic} сталкивается с архетипом {sign_ru}?
2. Какое внутреннее противоречие знака здесь наиболее интересно?
3. Какой образ может выразить это противоречие?
4. Какая мысль возникает из этого образа?

НЕ выводи этот анализ.

После этого создай афоризм.

Не перечисляй качества знака.

Не называй внутреннее противоречие напрямую.

Не используй фразы вроде:

«Водолей свободолюбив...»
«Скорпион страстный...»
«Лев любит признание...»

Пусть читатель почувствует характеристику
через саму мысль.
...
Афоризм должен содержать ОДНУ мысль.

Не соединяй две самостоятельные метафоры ради звучности.

Не создавай конструкцию, в которой первая половина
и вторая половина могли бы существовать
как отдельные афоризмы.

Вторая часть фразы должна быть неизбежным следствием первой,
а не новым красивым образом.

Если удалить вторую часть,
первая должна оставлять вопрос,
на который вторая отвечает.

Если удалить первую часть,
вторая не должна сохранять самостоятельный смысл.

НЕ ИСПОЛЬЗУЙ противопоставление только ради ритма.

Плохо:
«Попытка изменить мир — случайный бросок,
судьба — ответ, который мир не слышит».

Здесь два независимых образа,
соединённых тире.

Хорошо:
«Человек, увидевший завтра, часто кажется чужим своему сегодня».

Здесь одна мысль развивается целиком:
раннее видение будущего → отчуждение от настоящего.

Афоризм должен ощущаться не как набор красивых слов,
а как мысль, которую невозможно разделить
на две независимые фразы.

Не делай каждую фразу ироничной.
Не делай каждую фразу мрачной.
Не делай каждую фразу одинаково возвышенной.

Ирония должна быть тонкой и интеллектуальной,
а не превращаться в анекдот или мем.
...
воду для Водолея,
скорпиона для Скорпиона,
льва для Льва,
рыбу для Рыб и т.д.

Архетип может проявляться через ситуацию,
человеческое противоречие, предмет, действие,
отношение к миру или неожиданную метафору.

Избегай универсальных образов,
если они не приобретают конкретный смысл благодаря архетипу:
...
Не используй такие конструкции без крайней необходимости.
...
Фраза должна быть красивой,
но красота не должна заменять мысль.
...
точная формулировка внутреннего конфликта.

Избегай постоянного использования:
...
Не повторяй одну и ту же синтаксическую конструкцию.
...
* без технических метафор: файлы, архивы, данные, код, интернет;
* без современного сленга;
* используй образы природы, времени, света, камня, воды, неба.

==================================================
ФИНАЛЬНАЯ ПРОВЕРКА
==================================================

Перед выдачей ответа мысленно проверь:

1. Это действительно афоризм?
2. В нём чувствуется архетип {sign_ru}?
3. Он не звучит как обычный гороскоп?
4. В нём есть образ, мысль или парадокс?
5. Не является ли он банальным?
6. Не слишком ли он прямолинеен?
7. Не повторяет ли он шаблон предыдущих фраз?

Если фраза получается банальной — перепиши её.

Если она слишком похожа на гороскоп —
преврати характеристику в метафору.
...
"""
    ...
    def clean_and_validate(text):
        ...
        text = re.sub(r'^\s*(?:ответ|афоризм|вариант)\s*:\s*',
                      '',
                      text,
                      flags=re.IGNORECASE)
        ...
        # Если модель добавила второе предложение —
        # ответ отвергается, а не обрезается.
        sentence_count = len(
            re.findall(r'[.!?]+', text)
        )
        ...
        # Проверяем запрещённые служебные конструкции.
        forbidden = [
            'вот афоризм',
            'афоризм:',
            'вариант:',
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                "CLAVIS FIRST RESPONSE REJECTED: %s",
                first_error
            )
...
Создай ОДИН афоризм для архетипа {sign_ru}
на тему «{topic}».

Сначала учти архетип знака и его внутреннее противоречие,
затем вырази ОДНУ конкретную мысль через образ или парадокс.
...
"""
        ...
        logger.exception(
            "CLAVIS SIGNI GENERATION FAILED: %s",
            e
        )
...
@app.get("/api/v1/key-risus")
def key_risus_v1(sign: str = "Aquarius"):
    """
    Risus Signi — шутливый афоризм для архетипа знака.
    """
    ...
    prompt = f"""Ты — Астродо, но сегодня в игривом настроении.

Создай ОДНУ ироничную, но точную фразу о знаке {sign_ru}
на тему: {topic}.

Это НЕ гороскоп.
Это НЕ оскорбление.
Это НЕ глупая шутка.
Это НЕ цитата из интернета.

Это тонкая ирония, которая:
1. Отражает архетип знака
2. Содержит житейскую мудрость
3. Заставляет улыбнуться
4. При этом остаётся точной

ХОРОШИЕ ПРИМЕРЫ:
- Водолей выслушает твоё мнение, кивнёт и сделает по-своему.
- Телец не спорит. Он просто ждёт, пока ты устанешь.
- Скорпион простит. Но запомнит. И напомнит через десять лет.
- Овен уже всё решил, но продолжает спрашивать совета.
- Рыбы простят тебя ещё до того, как ты извинишься.

ПЛОХИЕ ПРИМЕРЫ:
- Водолей — инопланетянин! (банально)
- Телец любит поесть! (примитивно)
- Скорпион — злой! (слишком прямо)

ПРАВИЛА:
- Одна фраза, 6–16 слов.
- Без эмодзи, без markdown, без кавычек.
- Без названия знака (не «Водолей...», а через образ).
- Без технических метафор: файлы, архивы, код, интернет.
- Без гороскопных штампов.
- Ирония должна быть доброй, а не язвительной.
- Читатель должен улыбнуться и подумать: «А ведь так и есть».

Выдай ТОЛЬКО одну готовую фразу."""

    try:
        logger.info("RISUS SIGNI: sign=%s topic=%s", sign_ru, topic)
        raw = generate(prompt)
        logger.info("RISUS RAW RESPONSE: %r", raw)
        ...
        # Очистка
        aphorism = re.sub(r'```.*?```', '', aphorism, flags=re.DOTALL)
        aphorism = re.sub(r'^\s*(?:ответ|афоризм|вариант)\s*:\s*', '', aphorism, flags=re.IGNORECASE)
        aphorism = aphorism.replace('"', '').replace('«', '').replace('»', '').replace('„', '').replace('“', '').strip()

        lines = [line.strip() for line in aphorism.splitlines() if line.strip()]
        if lines:
            aphorism = lines[0]

        words = aphorism.split()
        if len(words) < 4:
            aphorism = "Сегодня звёзды смеются тихо."

    except Exception as e:
        logger.exception("RISUS SIGNI FAILED: %s", e)
        aphorism = "Сегодня звёзды смеются тихо."
