...
Стиль:
- РОВНО ОДНО ПРЕДЛОЖЕНИЕ (без переносов)
- Иронично, но не зло
- Философски, с бытовой мудростью
- В характере знака {sign_ru}
- Как Шопенгауэр, но для знака зодиака
- Никаких «вас ждёт», «будьте осторожны»

Пример того, как должен звучать афоризм для Водолея:
...
- Без заголовков
- Без markdown
- Тема — это отправная точка, ты можешь обыграть её как угодно

Не используй markdown. Не добавляй заголовков. Только текст афоризма одной строкой."""
