...
def key_v1(sign: str = "Aquarius"):
    ...
    Напиши короткий афоризм для знака {sign_ru} на тему: {topic}.

    Правила:
    - ОДНА КОРОТКАЯ СТРОКА (максимум 10 слов)
    - Иронично, философски, в характере знака
    - Без кавычек и переносов

    Пример: «Водолей не прощается — он просто перестаёт ждать лифт.»

    Выдай ТОЛЬКО одну короткую строку. Ничего другого."""
